  ...
  const signIn = () => {
    ...
    fetch('/auth/login', {
      ...
      if (body.resultCode == "200") {
        login({ userName: body.userName, userRole: body.userRole });
        toast.success('Login successful!');
        navigate("/");
      } else if (body.resultCode == "401") {
    ...
    <StyledWrapper>
      <div className="form">
        ...
        <div className="flex-row">
          <div>
            <input id="remember-me" type="checkbox" ref={rememberMeRef} />
            <label htmlFor="remember-me"> Remember me </label>
          </div>
          <Link to="/auth/forgot" className="span">Forgot password?</Link>
        </div>

        <button className="button-submit" onClick={signIn}>Sign In</button>

        <p className="p">Don't have an account? <Link to="/auth/register" className="span">Sign Up</Link>
        </p>
      </div>
    </StyledWrapper>
  );
